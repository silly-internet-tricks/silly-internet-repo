...
`, 'pokemon-card');
 ...
 document.addEventListener('keydown', async ({ code }) => {
  if (code === 'KeyP') {
   ...
   if (pokemonNames) {
    const urls: string[] = pokemonNames.map((pokemonName) => `https://pokeapi.co/api/v2/pokemon-species/${pokemonName}`);
    ...
    pokemonNames.forEach((pokemonName) => {
     if (pokemonCards.has(pokemonName)) {
      ...
      // @ts-expect-error I'm getting confetti from the cdn, not importing it
      confetti({
       origin: {
     ...
     const pokemonImageDiv: Element = document.createElement('div');
...
<div class="image" style="background-image: url('https://www.smogon.com/dex/media/sprites/xy/${pokemonName}.gif');"></div>`;
    ...
    try {
     const pokeResponseGenerator:
     AsyncGenerator<PokeResponse, void, PokeResponse> = (async function* pokeResponseGenerator() {
     ...
     for await (const { response } of pokeResponseGenerator) {
      const flavorTextContainer: HTMLElement = document.querySelector(`#flavor-text-container-${response.name}`);
      const englishFlavorText: { flavor_text: string }[] = response.flavor_text_entries.filter((e) => e.language.name === 'en');
