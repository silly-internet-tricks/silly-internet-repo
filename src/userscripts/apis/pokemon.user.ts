...
// @connect      pokeapi.co
// @source       https://github.com/silly-internet-tricks/silly-internet-repo/blob/main/src/userscripts/apis/pokemon.user.ts
...
const pokemonMegaRegExp: RegExp =
    ...
    pokemonNames.forEach((pokemonName) => {
     if (pokemonCards.has(pokemonName)) {
      const { x, y, width, height } = document
       .querySelector(`#pokemon-image-${pokemonName}`)
...
<div class="image" style="background-image: url('https://www.smogon.com/dex/media/sprites/xy/${pokemonName}.gif');"></div>`;
    ...
    try {
     type PokeResponseGenerator = AsyncGenerator<PokeResponse, void, PokeResponse>;
     ...
     for await (const { response } of pokeResponseGenerator) {
      const flavorTextContainer: HTMLElement = document.querySelector(
       `#flavor-text-container-${response.name}`,
      );
      const englishFlavorText: { flavor_text: string }[] = response.flavor_text_entries.filter(
       (e) => e.language.name === 'en',
      );
      makeCarousel(
       flavorTextContainer,
       englishFlavorText.map(({ flavor_text: flavorText }) => {
