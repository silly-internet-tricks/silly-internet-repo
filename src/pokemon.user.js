...
(function pokemon() {
 document.addEventListener('keydown', async ({ code }) => {
  if (code === 'KeyP') {
   ...
   const pokemonNames = selected.match(pokemonMegaRegExp);
   if (pokemonNames) {
    const urls = selected.match(pokemonMegaRegExp).map((e) => `https://pokeapi.co/api/v2/pokemon-species/${e.toLocaleLowerCase()}`);
    const pokemonCardContainer = document.querySelector('#pokemon-card-container') || document.createElement('div');
    ...
    pokemonNames.forEach((pokemonName) => {
     if (pokemonCards.has(pokemonName)) {
      const {
       x, y, width, height,
      } = document.querySelector(`#pokemon-image-${pokemonName.toLocaleLowerCase()}`).getBoundingClientRect();
...
<div id="pokemon-image-${pokemonName.toLocaleLowerCase()}">
<h3>${pokemonName.replace(/^(.)/, (_, p1) => p1.toLocaleUpperCase())}</h3>
<div style="background-image: url('https://www.smogon.com/dex/media/sprites/xy/${pokemonName.toLocaleLowerCase()}.gif');"></div>
</div>
`;
