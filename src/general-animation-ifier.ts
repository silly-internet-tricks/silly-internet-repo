...
import {
 makeInlineInlineBlock,
 undoInlineInlineBlock,
} from './make-inline-inline-block';
...
export default function generalAnimationifier(
 ...
 const undoHandler: (target: Node) => void = (target: Node) => {
  if (target) {
   ...
   if (!elementTarget.classList) {
    throw new Error(
     `expected the target (${elementTarget}) to be an element with a class list`,
    );
   }
 ...
 const undoEventHandler: (event: Event) => void = function undoEventHandler({
  target,
 }) {
 ...
 holdKeyAndClick(
  {
   do: ({ target }) => {
    ...
    if (!htmlElement.classList) {
     throw new Error(
      `expected the event target ${htmlElement} to be an HTMLElement with a classList`,
     );
    }

    if (!htmlElement.style) {
     throw new Error(
      `expected the event target ${htmlElement} to be an HTMLElement with a style`,
     );
    }
